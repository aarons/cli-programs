...
async fn main() -> Result<()> {
    ...
    let voice_ref = args.voice.or(config.voice_ref);
    ...
    let backend = tts::create_backend(args.device.as_deref(), voice_ref)?;
    ...
    while let Some((chapter_id, chunk_id)) = session::get_next_chunk(&session) {
        ...
        {
            Ok(()) => {
                session::mark_chunk_complete(&mut session, chapter_id, chunk_id, &audio_path)?;
            }
            Err(e) => {
                session::mark_chunk_error(&mut session, chapter_id, chunk_id, &e.to_string())?;

...
const SYSTEM_PROMPT: &str = r#"You are a filename cleaner for ebook files. Given an epub filename, extract and format it as:
...
- If series info isn't clear, omit it
- Keep titles concise - use shortened versions if the original is truncated
...
async fn process_file(
    ...
    let cleaned_name = llm.complete(original_name, SYSTEM_PROMPT).await?;
    let cleaned_name = sanitize_filename(cleaned_name.trim());

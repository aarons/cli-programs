...
/// Generate commit prompt for summary mode (when diff is too large)
/// Uses file list and user-provided context instead of full diff
pub fn generate_commit_prompt_summary_mode(context: &str) -> String {
    format!(
        r#"Please write a clear message that describes the changes in this pull request.

IMPORTANT: The diff was too large to include, so you are working with a file list and user-provided context instead of the actual code changes. Base your commit message on the available information.
...
Important:
- Work on the message draft until it is very concise and clear. Brevity with clarity is difficult to achieve but is important to strive for.
- Since you don't have the actual diff, rely heavily on the user's description and the file change patterns.
...
<observations>
Observations about the changes based on the file list and user context
Iterations on the message until it is clear and concise
</observations>
<commit_message>
commit-type: a description of the commit
